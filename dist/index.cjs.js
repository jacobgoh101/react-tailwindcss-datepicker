...
const Days = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
    ...
    return (React__default["default"].createElement("div", { className: "grid grid-cols-7 gap-y-0.5 my-1" },
        calendarData.days.previous.map((item, index) => (React__default["default"].createElement("button", { type: "button", key: index, className: "flex items-center justify-center text-gray-400 w-full h-12 lg:w-10 lg:h-10", onClick: () => onClickPreviousDays(item), onMouseOver: () => {
                hoverDay(item, "previous");
            } }, item))),
        calendarData.days.current.map((item, index) => (React__default["default"].createElement("button", { type: "button", key: index, className: buttonCass(item), onClick: () => {
                onClickDay(item);
            }, onMouseOver: () => {
                hoverDay(item, "current");
            } }, item))),
        calendarData.days.next.map((item, index) => (React__default["default"].createElement("button", { type: "button", key: index, className: "flex items-center justify-center text-gray-400 w-full h-12 lg:w-10 lg:h-10", onClick: () => {
...
const Input = () => {
    // Context
    const { primaryColor, period, dayHover, changeDayHover, calendarContainer, arrowContainer, inputText, changeInputText, hideDatepicker, changeDatepickerValue, asSingle, placeholder, separator, disabled, inputClassName } = React.useContext(DatepickerContext);
    // UseRefs
    ...
    return (React__default["default"].createElement(React__default["default"].Fragment, null,
        React__default["default"].createElement("input", { ref: inputRef, type: "text", className: getClassName(), disabled: disabled, placeholder: placeholder
                ? placeholder
